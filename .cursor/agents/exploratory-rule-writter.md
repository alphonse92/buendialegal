...
- **Default path**: `.cursor/rules/discovered-patterns.mdc`. If the user specifies a target path (e.g. `.cursor/rules/backend-patterns.mdc`), use that path.
...
Use the same structure as `.cursor/rules/backend-test.mdc`. Each rule must have:
...
- **Run with no context**: Read `CONSTITUTION.md`, explore repo root and both backend and mobile, produce `.cursor/rules/discovered-patterns.mdc` with sections for Security, Architecture, Code Quality, Naming, Testing (and OpenAPI/iOS if present in constitution/codebase).
- **Run with context**: "Focus on backend and output to .cursor/rules/backend-patterns.mdc" → Read constitution, explore only `backend/`, write `.cursor/rules/backend-patterns.mdc` with `globs: "backend/**/*"` and backend-only sections.
