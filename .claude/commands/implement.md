...
- Call: `Skill(git-workflow)` for branch management
- The skill will:
...
- Call: `Skill(specification-management)` to read spec
...
**At checkpoint:**
- Call: `Skill(drift-detection)` for spec alignment
- Call: `Skill(constitution-validation)` if CONSTITUTION.md exists
...
- Call: `Skill(implementation-verification)` for final validation
